...
import { useCreateBusinessDirectoryMutation } from "@/redux/api/business/businessApi";
...
const CreateBusinessModal = ({ open, onClose, onCreated, form, tradeInfoData }: Props) => {
  const [createBusiness, { isLoading }] = useCreateBusinessDirectoryMutation();
  ...
  const handleBusinessKhatChange = (value: string) => {
    ...
    const selected = tradeInfoData?.find((d) => d.khat_id === value);
    if (selected) {
      setSelectedKhat(selected);
      setKhatAmounts(selected.khat_fees || []);
      form.setFieldsValue({
        applicant_type_of_businessKhat: value,
        applicant_type_of_businessKhatAmount: undefined,
      });
    }
  };
      ...
      >
        <div style={sectionStyle}>
          ...
          <Row gutter={16}>
            ...
            <Col span={8}><Form.Item name="applicant_date_of_birth" label="জন্ম তারিখ"><Input className="w-100" type="date" style={inputStyle} /></Form.Item></Col>
