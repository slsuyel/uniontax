...
import UpdatePermitDetailsModal from "./UpdatePermitDetailsModal"; // নতুন মডাল ইম্পোর্ট
...
export interface TTender {
  ...
  bankCheck: null | string;
  daysOfDepositeAmount: null | number;
  permitDetials: null | string;
  created_at: Date;
...
const TenderList = () => {
  ...
  // নতুন পারমিট আপডেট মডাল এর state
  const [permitModalVisible, setPermitModalVisible] = useState(false);
  const [permitSelectedTender, setPermitSelectedTender] = useState<TTender | null>(null);
  ...
  // Committee Modal open/close handlers
  ...
  // নতুন পারমিট আপডেট মডাল open/close handlers
  const openPermitModal = (tender: TTender) => {
    setPermitSelectedTender(tender);
    setPermitModalVisible(true);
  };
  const closePermitModal = () => {
    setPermitModalVisible(false);
    setPermitSelectedTender(null);
  };

  const handlePermitUpdated = () => {
    refetch(); // ডাটা রিফ্রেশ করো
  };
  ...
  return (
    <div className="card p-3 border-0">
                    ...
                    <tr key={tender.id || index}>
                      ...
                      <td className="text-center">
                        <div className="btn-group" role="group">
                          ...
                          <button type="button" className="btn btn-sm btn-outline-warning" title="ব্যাংক ও পারমিট তথ্য আপডেট করুন" onClick={() => openPermitModal(tender)}>
                            <i className="fas fa-edit me-1"></i> কার্যাদেশ আপডেট
                          </button>
      ...
      <CommitteeDetailsModal open={committeeDetailsModalVisible} onClose={closeCommitteeDetailsModal} selectedTender={selectedTender} />
      ...
      {/* Validate Committee Modal */}
      <ValidateCommitteeModal open={validateModalVisible} onClose={closeValidateCommitteeModal} selectedTender={selectedTender} loading={false} />

      {/* New Permit Update Modal */}
      <UpdatePermitDetailsModal
        open={permitModalVisible}
        onClose={closePermitModal}
        tenderId={permitSelectedTender?.id ?? null}
        initialData={permitSelectedTender}
