...
const SonodManagement = () => {
  const [sonod_Id, setSonod_Id] = useState(""); // For input field value
  const [searchSonodId, setSearchSonodId] = useState(""); // For query execution
  const { sonodName, condition } = useParams();
  const token = localStorage.getItem("token");
  const { data, isLoading } = useAllSonodQuery({
    sonodName: sonodName,
    stutus: condition || "Pending",
    sondId: searchSonodId, // Use the state updated only on button click
    token,
  });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSonod_Id(value); // Update input state

    if (value === "") {
      setSearchSonodId(""); // Reset query to fetch all data
    }
  };
