...
/* eslint-disable @typescript-eslint/no-explicit-any */
import { Form, Input, Select, Upload, Button, FormInstance } from "antd";
import { UploadOutlined } from "@ant-design/icons";
...
const RESIDENT_OPTIONS = ["স্থায়ী", "অস্থায়ী"];
const RELATION_OPTIONS = [
  "নিজ", "ভাড়াটিয়া", "পিতা", "মাতা", "স্ত্রী", "পুত্র",
  "কন্যা", "স্বামী", "ভাই", "বোন", "নাতি", "নাতনি"
];
const RELIGION_OPTIONS = ["ইসলাম", "হিন্দু", "খ্রিস্টান", "বৌদ্ধ", "অন্যান্য"];

const inputStyle = { height: 40, width: "100%" };

const CommonFields = ({ form, setFormData }: CommonFieldsProps) => {
  const initialValues = form.getFieldsValue();

  const renderInput = (label: string, name: string, rules: any[] = []) => (
    <div className="col-md-4">
  ...
  const renderSelect = (label: string, name: string, options: string[], rules: any[] = []) => (
    <div className="col-md-4">
      <Form.Item label={label} name={name} rules={rules}>
        <Select placeholder={`Select ${label}`} style={inputStyle}>
          {options.map(option => (
  ...
  return (
    <>
      {renderInput("আবেদনকারীর নাম", "applicant_name", [{ required: true, message: "Please enter your name" }])}
      {renderSelect("লিঙ্গ", "applicant_gender", GENDER_OPTIONS, [{ required: true, message: "এই তথ্যটি প্রয়োজন" }])}
      {renderInput("পিতা/স্বামীর নাম", "applicant_father_name", [{ required: true, message: "এই তথ্যটি প্রয়োজন" }])}
      {renderInput("মাতার নাম", "applicant_mother_name", [{ required: true, message: "এই তথ্যটি প্রয়োজন" }])}
      {renderInput("জাতীয় পরিচয়পত্র নং", "applicant_national_id_number")}
      {renderInput("জন্ম নিবন্ধন নং", "applicant_birth_certificate_number")}
      {renderSelect("বাসিন্দার ধরণ", "applicant_resident_status", RESIDENT_OPTIONS, [{ required: true, message: "এই তথ্যটি প্রয়োজন" }])}
      {renderInput("মোবাইল", "applicant_mobile", [
        { required: true, message: "দয়া করে আপনার মোবাইল নম্বর লিখুন" },
        { len: 11, message: "মোবাইল নম্বরটি ১১ অক্ষর হতে হবে" },
        { pattern: /^[0-9]+$/, message: "মোবাইল নম্বরটি শুধুমাত্র সংখ্যা হতে হবে" }
      ])}
      {renderInput("হোল্ডিং নং", "applicant_holding_tax_number")}
      
      <fieldset className="border rounded p-3 mb-3 col-md-12">
        <legend className="w-auto px-2" style={{ fontWeight: 'bold' }}>হোল্ডিং সম্পর্কিত তথ্য</legend>
        <div className="row mx-auto">
          {renderInput("হোল্ডিং মালিকের নাম", "holding_owner_name")}
          {renderSelect("হোল্ডিং মালিকের সাথে সম্পর্ক", "holding_owner_relationship", RELATION_OPTIONS)}
          {renderInput("হোল্ডিং মালিকের মোবাইল নম্বর", "holding_owner_mobile")}
        </div>
      </fieldset>
      ...
      <div className="col-md-4">
        <Form.Item label="জন্ম তারিখ (মাস/দিন/বছর)" name="applicant_date_of_birth" rules={[{ required: true, message: "এই তথ্যটি প্রয়োজন" }]}>
          <input
            type="date"
            className="form-control w-100"
            value={initialValues.applicant_date_of_birth || ""}
            onChange={(e) => {
              setFormData({ ...initialValues, applicant_date_of_birth: e.target.value });
            }}
          />
        </Form.Item>
      </div>
      ...
      <div className="col-md-4">
        <Form.Item label="আবেদনকারীর ছবি" name="image" valuePropName="file" getValueFromEvent={(e) => e?.file}>
          <Upload name="image" listType="picture" maxCount={1}>
            <Button icon={<UploadOutlined />}>Upload Image</Button>
          </Upload>
        </Form.Item>
      ...
      {renderSelect("ধর্ম", "applicant_religion", RELIGION_OPTIONS, [{ required: true, message: "এই তথ্যটি প্রয়োজন" }])}
    </>
  );
};
