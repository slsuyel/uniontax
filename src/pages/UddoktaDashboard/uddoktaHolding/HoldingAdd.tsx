...
import { useUddoktaAddHoldingMutation } from "@/redux/api/sonod/sonodApi";

const { Option } = Select;

const HoldingAdd = () => {
  const token = localStorage.getItem("token");
  const [uddoktaAddHolding, { isLoading }] = useUddoktaAddHoldingMutation();
  ...
  const onFinish = async (values: any) => {
    try {
      const res = await uddoktaAddHolding({ data: values, token }).unwrap();
